...
use crate::time::{Duration, SymbolsOQpsk250kB};
...
pub const A_BASE_SLOT_DURATION: Duration<SymbolsOQpsk250kB> = Duration::new(60);
/// The number of symbols forming a superframe when the superframe order is
/// equal to zero.
pub const A_BASE_SUPERFRAME_DURATION: Duration<SymbolsOQpsk250kB> =
    Duration::new(A_BASE_SLOT_DURATION.ticks() * A_NUM_SUPERFRAME_SLOTS as i64);
...
pub const A_MIN_CAP_LENGTH: Duration<SymbolsOQpsk250kB> = Duration::new(440);
...
/// 10.2.2 and 10.2.3.
pub const A_TURNAROUND_TIME: Duration<SymbolsOQpsk250kB> = Duration::new(12);
/// The time required to perform CCA detection in symbol periods.
pub const PHY_CCA_DURATION: Duration<SymbolsOQpsk250kB> = Duration::new(8);
/// The number of symbols forming the basic time period used by the CSMA-CA
/// algorithm.
pub const MAC_UNIT_BACKOFF_PERIOD: Duration<SymbolsOQpsk250kB> =
    Duration::new(A_TURNAROUND_TIME.ticks() + PHY_CCA_DURATION.ticks());
/// O-QPSK symbol rate for 2.4G is 62.5kS/s, i.e. a symbol period of 16µs.
/// SIFS: 12 symbols = 192µs
pub const MAC_SIFS: Duration<SymbolsOQpsk250kB> = Duration::new(12);
/// LIFS: 40 symbols = 480µs
pub const MAC_LIFS: Duration<SymbolsOQpsk250kB> = Duration::new(40);
/// AIFS=1ms, for SUN PHY, LECIM PHY, TVWS PHY, SIFS otherwise
pub const MAC_AIFS: Duration<SymbolsOQpsk250kB> = MAC_SIFS;
...
#[cfg(test)]
mod tests {
    use crate::time::{Duration, Microseconds, SymbolsOQpsk250kB};

    #[test]
    fn inv_symbol_rate() {
        let symbol_period: Duration<Microseconds> =
            Duration::<SymbolsOQpsk250kB>::new(1).convert_into_rounding_down();
        assert_eq!(symbol_period.ticks(), 16);
    }
}
