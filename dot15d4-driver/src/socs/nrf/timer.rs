...
use crate::{
    time::{Instant, Nanoseconds},
    Frequency, RadioTimerApi,
};
...
const _: () = assert!(
    NrfRadioTimer::ns_to_sleep_ticks(Instant::new(u64::MAX)) == NrfRadioTimer::MAX_RTC_TICKS
);
const _: () = {
    const ROUNDING_ERROR: u64 = 17924; // one RTC tick is ~30517ns, the rounding
                                       // error must be less
    assert!(
        NrfRadioTimer::sleep_ticks_to_ns(NrfRadioTimer::MAX_RTC_TICKS).tick()
...
impl NrfRadioTimer {
    ...
    const fn sleep_ticks_to_ns(sleep_ticks: u64) -> Instant<Nanoseconds> {
        ...
        //              = (ticks * (5^9 / 2^6)) ns
        //              = ((ticks * 5^9) >> 6) ns
        const _: () = assert!(<RtcDriver as Frequency>::FREQUENCY == 2_u32.pow(15));
        ...
        Instant::new(ns as u64)
    }

    const fn ns_to_sleep_ticks(ns: Instant<Nanoseconds>) -> u64 {
        // To keep tick-to-ns conversion cheap we avoid division while
        // minimizing rounding errors:
        //
        ...
        ((ns.tick() as u128 * MULTIPLIER) >> N) as u64
    }
}

impl RadioTimerApi for NrfRadioTimer {
    fn now() -> Instant<Nanoseconds> {
        Self::sleep_ticks_to_ns(RTC_DRIVER.now())
    }

    async fn wait_until(instant: Instant<Nanoseconds>) {
