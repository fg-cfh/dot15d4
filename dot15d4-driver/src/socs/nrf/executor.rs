...
use crate::executor::trace::TASK_INT;
...
impl State {
    ...
    fn on_interrupt(&self) {
        #[cfg(feature = "rtos-trace")]
        rtos_trace::trace::task_exec_begin(TASK_INT);
        ...
        //         pointer cannot dangle. Checking for null pointers is required to
        ...
        if let Some(task) = unsafe { task.as_mut() } {
            ...
            match task.as_mut().poll(&mut cx) {
                core::task::Poll::Ready(_) => {
                    if let Some(outer_waker) = unsafe { self.outer_waker.get().as_ref() }.unwrap() {
    ...
    async fn spawn<Task: Future<Output = ()>>(&self, task: Task) {
        ...
        poll_fn(|cx| {
            ...
            let outer_waker = unsafe { self.outer_waker.get().as_ref() }.unwrap();
            ...
            if let Some(outer_waker_ref) = outer_waker {
                if task_ptr.is_null() {
                    ...
                    //         re-check here.
                    unsafe { self.outer_waker.get().as_mut() }.unwrap().take();
                ...
                //         modification.
                unsafe { self.outer_waker.get().as_mut() }
                    .unwrap()
...
macro_rules! nrf_interrupt_executor {
    ($mod:ident, $interrupt:ident, $swi:ident) => {
        pub mod $mod {
            use core::{
                future::Future,
                ptr,
            ...
            use nrf52840_hal::pac::{interrupt, $swi, Interrupt};
            ...
            impl NrfInterruptExecutor {
                /// Mutability proves exclusive ownership of the executor. Ownership of the
                /// software interrupt is transferred to the executor.
                pub(super) fn init(
                    &'static mut self,
                    _swi: $swi,
        ...
        /// Safety: Transferring ownership of the SWI peripheral proves that only a
        ///         single instance of the executor can be requested.
        #[cfg(not(feature = "gpio-trace"))]
        pub fn $mod(swi: nrf52840_hal::pac::$swi) -> &'static mut $mod::NrfInterruptExecutor {
            $mod::EXECUTOR.init($mod::NrfInterruptExecutor).init(swi, 0)
        }

        #[cfg(feature = "gpio-trace")]
        pub fn $mod(
            swi: nrf52840_hal::pac::$swi,
            _gpiote: &GPIOTE,
            gpiote_trace_channel: usize,
        ) -> &'static mut $mod::NrfInterruptExecutor {
            $mod::EXECUTOR
                .init($mod::NrfInterruptExecutor)
                .init(swi, gpiote_trace_channel)
        }
    };
}
