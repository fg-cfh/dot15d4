...
const FRAME_PERIOD: SyntonizedDuration = SyntonizedDuration::millis(500);
...
async fn main(spawner: Spawner) {
    ...
    let mut tx_count = 0;
    let anchor_time = now::<NrfRadioTimer>();
    ...
    loop {
        ...
        if addr == 1 {
            ...
            unsafe {
                wait_until::<NrfRadioTimer>(anchor_time + ((tx_count + 1) * FRAME_PERIOD)).await;
            }
        }
    }
}
