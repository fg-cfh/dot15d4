...
use crate::{
    driver::DriverConfig,
    mpdu::{
        MpduWithAddressing, MpduWithAllFields, MpduWithFrameControl, MpduWithIes, MpduWithSecurity,
    },
    repr::{AddressingRepr, SeqNrRepr},
    Error, Result,
};

use super::{ParsedUpToAddressing, ParsedUpToSecurity};
...
pub(crate) struct ParserInfo<State> {
...
impl ParserInfo<MpduWithFrameControl> {
    pub(crate) const fn new(offset_mpdu: u16, seq_nr: SeqNrRepr) -> Self {
        const FRAME_CONTROL_LEN: u8 = 2;
        const SEQ_NR_LEN: u8 = 1;
        let offset_frame_control = offset_mpdu as u8;
    ...
    ) -> Result<ParserInfo<MpduWithAddressing>> {
    ...
    pub(crate) const fn without_addressing(&self) -> ParserInfo<MpduWithAddressing> {
        self.next_state(self.offset_addressing.get())
    }

    const fn next_state(&self, next_offset: u8) -> ParserInfo<MpduWithAddressing> {
        #[cfg(feature = "security")]
        let offset_ies = None;
        ...
        #[cfg(not(all(feature = "security", feature = "ies")))]
        let offset_frame_payload = as_nz_u8(next_offset);
        ParserInfo {
            state: PhantomData,
...
impl ParserInfo<MpduWithAddressing> {
    #[cfg(feature = "security")]
    pub(crate) const fn with_security(
        &self,
        security: SecurityRepr,
    ) -> ParserInfo<MpduWithSecurity> {
    ...
    pub(crate) const fn without_security(&self) -> ParserInfo<MpduWithSecurity> {
        #[cfg(feature = "security")]
        return self.next_state(self.offset_aux_sec_hdr.unwrap().get(), None);
        #[cfg(not(feature = "security"))]
        return self.next_state(self.offset_addressing.unwrap().get(), None);
    }
...
impl ParserInfo<MpduWithSecurity> {
    ...
    #[cfg(feature = "ies")]
    pub(crate) const fn with_ies_and_payload_length<'ies, Config: DriverConfig>(
        &self,
        ies: IeListReprVariant,
        frame_payload_length: u16,
    ) -> Result<ParserInfo<MpduWithAllFields>> {
    ...
    #[cfg(feature = "ies")]
    pub(crate) const fn with_ies_and_mpdu_length<'ies, Config: DriverConfig>(
        &self,
        ies: IeListReprVariant,
    ...
    ///
    /// See [`crate::driver::RadioFrameRepr::sdu_wo_fcs_length()`].
    pub(crate) const fn without_ies_with_mpdu_length<'ies, Config: DriverConfig>(
        &self,
        mpdu_length_wo_fcs: u16,
    ) -> Result<ParserInfo<MpduWithAllFields>> {
    ...
    const fn last_offset(&self) -> u16 {
        ...
        let last_offset = self.offset_aux_sec_header.unwrap().get();
        #[cfg(not(all(feature = "security", feature = "ies")))]
        let last_offset = self.offset_addressing.unwrap().get();
        last_offset as u16
    }
...
impl ParserInfo<MpduWithIes> {
    #[cfg(feature = "ies")]
    pub(crate) const fn with_ies<'ies, Config: DriverConfig>(
        &self,
        ies: IeListReprVariant,
        frame_payload_length: u16,
    ) -> Result<ParserInfo<MpduWithAllFields>> {
        let ies_length = match ies.ies_length(frame_payload_length > 0) {
            Ok(ies_length) => ies_length,
            Err(e) => {
                return Err(e);
            }
        };
    ...
    pub(crate) const fn without_ies<Config: DriverConfig>(
        &self,
        frame_payload_length: u16,
    ) -> ParserInfo<MpduWithAllFields> {
        self.next_state::<Config>(0, frame_payload_length)
...
impl<State: ParsedUpToAddressing> ParserInfo<State> {
...
/// [`ParsedUpToSecurity`].
impl<State: ParsedUpToSecurity> ParserInfo<State> {
...
/// to all sub-fields.
impl ParserInfo<MpduWithAllFields> {
    ...
    pub(crate) const fn range_frame_payload(&self) -> Option<Range<usize>> {
        let offset_frame_payload = self.offset_frame_payload.unwrap().get() as usize;
        #[cfg(feature = "security")]
        let next_offset =
            self.offset_fcs.unwrap().get() as usize - self.length_mic.unwrap().get() as usize;
        #[cfg(not(feature = "security"))]
        let next_offset = self.offset_fcs.unwrap().get() as usize;
        if next_offset == offset_frame_payload {
            None
        } else {
            Some(offset_frame_payload..next_offset)
        }
    }
    ...
    /// The index of the first byte of headroom. We don't return a range as
    /// interpreting headroom is only done on radio frames.
    pub(crate) const fn offset_headroom(&self) -> u16 {
